...
export default class PScriptParserBase extends antlr4.Parser {
  constructor(input) {
    super(input);
  }
  ...
  notLineTerminator() {
    return !this.here(PScriptParser.LineTerminator);
  }

  notOpenBraceAndNotFunction() {
    const nextTokenType = this._input.LT(1).type;
    return (
      nextTokenType !== PScriptParser.OpenBrace &&
      nextTokenType !== PScriptParser.Function_
    );
  }

  closeBrace() {
    return this._input.LT(1).type === PScriptParser.CloseBrace;
  }
  ...
  here(type) {
    ...
    const nextTokenIndex =
      currentToken == null ? 0 : currentToken.tokenIndex + 1;
  ...
  lineTerminatorAhead() {
    ...
    if (ahead.type === PScriptParser.LineTerminator) {
      return true;
    }
    if (ahead.type === PScriptParser.WhiteSpaces) {
    ...
    return (
      (type === PScriptParser.MultiLineComment &&
        (text.includes("\r") || text.includes("\n"))) ||
      type === PScriptParser.LineTerminator
    );
  }
}
